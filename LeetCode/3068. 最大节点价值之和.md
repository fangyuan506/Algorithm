...
题目链接：[3068. 最大节点价值之和 - 力扣（LeetCode）](https://leetcode.cn/problems/find-the-maximum-sum-of-node-values/description/)
...
class Solution {
    public long maximumValueSum(int[] nums, int k, int[][] edges) {
        long f = 0, g = Long.MIN_VALUE;
        for (int num : nums) {
            int m = num ^ k;
            long g0 = g;
            g = Math.max(g0 + num, f + m);
            f = Math.max(f + num, g0 + m);
        }
        return f;
    }
}
